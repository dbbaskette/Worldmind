...
export function useMissionList() {
  ...
  const submitMission = useCallback(async (request: string, mode: string, projectPath?: string, gitRemoteUrl?: string, reasoningLevel?: string, executionStrategy?: string, createCfDeployment?: boolean, prdDocument?: string, skipPerTaskTests?: boolean): Promise<string> => {
    setSubmitting(true)
    setSubmitError(null)

    try {
      const result = await apiClient.submitMission(request, mode, projectPath, gitRemoteUrl, reasoningLevel, executionStrategy, createCfDeployment, prdDocument, skipPerTaskTests)
