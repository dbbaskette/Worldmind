...
class WorktreeExecutionContextTest {
    ...
    void acquireWorktreeCreatesWorktree() {
        ...
        Path worktreePath = Path.of("/tmp/worktree-TASK-001");

        when(gitWorkspaceManager.createMissionWorkspace(anyString(), anyString()))
                .thenReturn(workspacePath);
        when(gitWorkspaceManager.addWorktree(workspacePath, "TASK-001", "main"))
                .thenReturn(GitWorkspaceManager.WorktreeResult.success(worktreePath));

        context.createMissionWorkspace("MISSION-001", "https://github.com/test/repo.git");
        Path result = context.acquireWorktree("MISSION-001", "TASK-001", "main");

        assertEquals(worktreePath, result);
        verify(gitWorkspaceManager).addWorktree(workspacePath, "TASK-001", "main");
    }
    ...
    void acquireWorktreeReusesExistingWorktree() {
        ...
        context.createMissionWorkspace("MISSION-001", "https://github.com/test/repo.git");
        context.acquireWorktree("MISSION-001", "TASK-001", "main");
        context.acquireWorktree("MISSION-001", "TASK-001", "main");
    ...
    @Test
    void getWorktreeReturnsNullForUnknownTask() {
    ...
    void commitAndPushDelegatesToGitManager() {
        ...
        when(gitWorkspaceManager.addWorktree(any(), any(), any()))
                .thenReturn(GitWorkspaceManager.WorktreeResult.success(worktreePath));
        when(gitWorkspaceManager.commitAndPushWorktree(worktreePath, "TASK-001"))
                .thenReturn(true);

        context.createMissionWorkspace("MISSION-001", "https://github.com/test/repo.git");
        context.acquireWorktree("MISSION-001", "TASK-001", "main");
        boolean result = context.commitAndPush("TASK-001");

        assertTrue(result);
        verify(gitWorkspaceManager).commitAndPushWorktree(worktreePath, "TASK-001");
    }

    @Test
    void commitAndPushReturnsFalseForUnknownTask() {
    ...
    void releaseWorktreeRemovesWorktree() {
        ...
        context.createMissionWorkspace("MISSION-001", "https://github.com/test/repo.git");
        context.acquireWorktree("MISSION-001", "TASK-001", "main");
        context.releaseWorktree("MISSION-001", "TASK-001");

        verify(gitWorkspaceManager).removeWorktree(workspacePath, "TASK-001");
        assertNull(context.getWorktree("TASK-001"));
    }
    ...
    void getActiveWorktreeCountReturnsCorrectValue() {
        ...
        when(gitWorkspaceManager.createMissionWorkspace(anyString(), anyString()))
                .thenReturn(workspacePath);
        when(gitWorkspaceManager.addWorktree(any(), eq("TASK-001"), any()))
                .thenReturn(GitWorkspaceManager.WorktreeResult.success(Path.of("/tmp/wt-1")));
        when(gitWorkspaceManager.addWorktree(any(), eq("TASK-002"), any()))
        ...
        context.acquireWorktree("MISSION-001", "TASK-002", "main");
