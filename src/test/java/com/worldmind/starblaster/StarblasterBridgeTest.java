...
class StarblasterBridgeTest {
    ...
    void executeDirectiveReturnsUpdatedDirectiveOnSuccess() {
        ...
        when(manager.executeDirective(
            eq("FORGE"), eq("DIR-001"), any(), anyString(), any(), any(), any()))
            .thenReturn(execResult);

        var result = bridge.executeDirective(directive, context, Path.of("/tmp/p"), "", "base");
    ...
    void executeDirectiveMarksFailedOnNonZeroExit() {
        ...
        when(manager.executeDirective(any(), any(), any(), anyString(), any(), any(), any()))
            .thenReturn(execResult);

        var result = bridge.executeDirective(directive, null, Path.of("/tmp"), "", "base");
